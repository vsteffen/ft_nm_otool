...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/09/06 19:55:02 by vsteffen          #+#    #+#             */
/*   Updated: 2018/09/06 19:55:03 by vsteffen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_nm_otool.h"

int8_t				init_get_sym_32(struct s_get_sym_32 *get_sym, \
...
int8_t				add_sym_elem_32(struct s_get_sym_32 *get_sym, \
	struct s_nm_data *nm_data, void *ptr_header, int8_t endian)
{
	get_sym->sym_elem = (struct s_sym_32 *)malloc(sizeof(struct s_sym_32));
...
struct s_sym_32		*get_sym_list_32(void *ptr_header, \
	...
	struct s_nm_data *nm_data)
{
	struct s_get_sym_32		get_sym;

	get_sym.endian = endian;
	if (init_get_sym_32(&get_sym, ptr_header, sym_command, nm_data) == 0)
		return (NULL);
	while (get_sym.i < get_sym.sym_command_nsyms)
	{
		if (add_sym_elem_32(&get_sym, nm_data, ptr_header, endian) == 0)
			return (NULL);
	}
	if (get_sym.i > 0)
...
void				add_sect_32_elem(struct s_add_sect_32 *add_sect, \
	struct s_nm_32 *nm_32, size_t *count_sect, struct s_nm_data *nm_data)
{
	add_sect->sect_elem = (struct s_sect_32 *)malloc(sizeof(struct s_sect_32));
	...
	if (!nm_32->sect_list)
		nm_32->sect_list = add_sect->sect_elem;
	(*count_sect)++;
	nm_data->ti++;
}
	...
	struct s_nm_data *nm_data)
{
	...
	while (nm_data->ti < seg_nsects)
		add_sect_32_elem(add_sect, nm_32, count_sect, nm_data);
