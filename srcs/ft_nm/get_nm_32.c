...
struct s_sym_32			*get_sym_list_32(char *content, struct symtab_command *sym_command)
{
...
struct s_sect_32	*add_sect_32_to_list(struct s_nm_32 *nm_32, struct segment_command *seg, size_t *count_sect, struct s_sect_32 *sect_last_list)
{
...
struct s_nm_32		*get_nm_32(char *content)
{
	size_t						i;
	size_t						count_sect;
	...
	while (i++ < header->ncmds)
	{
		if (lc->cmd == LC_SYMTAB)
		{
			...
			nm_32->sym_list = get_sym_list_32(content, sym_command);
			nm_32->sym_list_size = sym_command->nsyms;
		}
		if (lc->cmd == LC_SEGMENT)
			sect_last_list = add_sect_32_to_list(nm_32, (struct segment_command *)lc, &count_sect, sect_last_list);
