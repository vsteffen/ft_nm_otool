...
int8_t		iter_sym_table_and_print_sect_32(struct s_nm_32 *nm_32, \
	struct s_sym_32 *sym, int8_t stab_ext_p_local)
{
...
int8_t		iter_sym_table_and_print_value_32(struct s_nm_32 *nm_32, \
	struct s_sym_32 *sym, int8_t stab_ext_p_local, int8_t real_type)
{
	if ((real_type & N_SECT) == N_SECT)
	{
		if (iter_sym_table_and_print_sect_32(nm_32, sym, stab_ext_p_local) \
			== -1)
			return (-1);
	}
...
int8_t		iter_sym_table_and_print_32(struct s_nm_32 *nm_32, int8_t endian)
{
	...
	while (sym)
	{
		...
		{
			if (real_type != N_UNDF)
				ft_printf("%08llx", (endian) \
					? endian_swap_int32(sym->elem.n_value) : sym->elem.n_value);
			else
				ft_putstr("        ");
			if (iter_sym_table_and_print_value_32(nm_32, sym, stab_ext_p_local,\
				real_type) == -1)
				return (-1);
